							...
							<div className="lg:col-span-3 space-y-5">
								...
								<Card title={role === 'lend' ? 'What you lend' : 'What you borrow'}>
									<TokenAmountRow
										...
										allowCustom={networkConfig.isPlayground}
									/>
...
function TokenSelect({ tokens, value, onSelect, balances, tokenInfo, allowCustom = false }: {
	...
	// Custom token addresses are only offered on the testnet playground. Mainnets are pinned to the
	// curated ETH/USDC list so the interface never invites a fake/garbage token; the underlying
	// contract stays fully permissionless for anyone calling it directly.
	allowCustom?: boolean;
}) {
				...
				<>
					...
					<div className="absolute z-30 mt-1 right-0 w-64 bg-white border border-gray-200 rounded-xl shadow-lg p-1.5">
						...
						{allowCustom ? (
							...
							<p className="px-2 py-1.5 text-[11px] text-gray-400 leading-snug">ETH &amp; USDC only on mainnet for now &mdash; custom tokens are enabled on the Sepolia testnet.</p>
						)}
					</div>
				</>
			)}
...
function TokenAmountRow({ label, tokens, value, onSelect, info, balances, tokenInfo, amount, setAmount, showSlider, balance, allowCustom = false }: {
	...
	return (
		<div className="bg-gray-50 rounded-xl p-3">
			...
			<div className="flex items-center gap-2">
				...
				<TokenSelect tokens={tokens} value={value} onSelect={onSelect} balances={balances} tokenInfo={tokenInfo} allowCustom={allowCustom} />
			</div>
			{hasBal && (
				<div className="mt-2">
